...
export async function scrapeGameTrex(url) {
  ...
  try {
    ...
    let description = '';
    // Look for content after "About [Game Name]" up to "How to Download..."
    ...
    $('.entry-content').children().each((i, el) => {
      ...
      if (tagName.match(/^h[2-4]$/) && text.toLowerCase().includes('about')) {
        foundAbout = true;
        return; // skip the 'About' heading
      }
      
      // Stop if we hit how to download
      ...
      if (foundAbout) {
        // Ignore system requirements list if it appears early
        if (tagName === 'ul' && text.toLowerCase().includes('os:')) {
            return; // continue
        }
        
        // Remove img tags so images don't appear in description, they should only be in the Media section
        $el.find('img').remove();
        
        // Append the raw HTML of the element to preserve formatting like lists and paragraphs
        description += $.html($el);
      }
    });
    ...
    if (!description) {
      $('.entry-content p').each((i, el) => {
        ...
        if (text && text.length > 50 && !text.includes('OS:') && !text.toLowerCase().includes('download')) {
          ...
          return false; // Break after first good paragraph
        }
      });
    }
