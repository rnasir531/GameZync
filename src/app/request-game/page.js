...
export default function RequestGamePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ text: 'Your request has been submitted successfully! We will review it soon.', type: 'success' });
        e.target.reset();
      } else {
        setMsg({ text: result.error || 'Failed to submit request', type: 'danger' });
      }
    } catch (err) {
  ...
  return (
    <section className="static-page-view" id="request-game-view" style={{ maxWidth: '800px', margin: '0 auto', padding: '20px 15px' }}>
      {/* Sleek Cyber Header */}
      <div className="static-page-header" style={{ textAlign: 'center', marginBottom: '32px' }}>
        ...
        <p className="static-page-subtitle" style={{ color: 'var(--text-muted)', fontSize: '15px', margin: 0 }}>
          Can't find the game you're looking for? Request it and our team will add it!
        </p>
      </div>
      ...
      >
        <form id="request-game-form" onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '20px', marginBottom: '20px' }}>
            <div className="form-group">
              <label style={{ display: 'block', color: 'var(--text-color)', fontWeight: '700', fontSize: '13.5px', marginBottom: '8px' }}>
                Game Name <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input 
                type="text" 
                name="game_name" 
                required 
                placeholder="Exact name of the game"
                style={{ 
                  width: '100%', 
                  padding: '12px 16px', 
                  borderRadius: '12px', 
            ...
            <div className="form-group">
              <label style={{ display: 'block', color: 'var(--text-color)', fontWeight: '700', fontSize: '13.5px', marginBottom: '8px' }}>
                Your Email <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>(Optional)</span>
              </label>
              <input 
                type="email" 
                name="email" 
                placeholder="So we can notify you when added"
                style={{ 
                  width: '100%', 
                  padding: '12px 16px', 
                  borderRadius: '12px', 
          ...
          <div className="form-group" style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', color: 'var(--text-color)', fontWeight: '700', fontSize: '13.5px', marginBottom: '8px' }}>
              Additional Details / DLCs <span style={{ color: '#ef4444' }}>*</span>
            </label>
            <textarea 
              name="detail" 
              rows="4" 
              required 
              placeholder="Any specific game version, update, DLCs, or platform details..."
              style={{ 
                width: '100%', 
                padding: '14px 16px', 
                borderRadius: '14px', 
