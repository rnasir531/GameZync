...
export default function HardwareMatchScoreCard({ userSpecs, matchResult, isTesting, handleRunTest }) {
  ...
  // 1. COMPACT SLEEK MOBILE CARD WITH SMOOTH SPINNING SATELLITE DISH ICON
  if (isMobile) {
    return (
      <div 
      ...
      >
        ...
        >
          <i className="fa-solid fa-satellite-dish fa-spin" style={{ fontSize: '18px', color: '#10b981', animationDuration: '4s' }}></i>
        </div>
        <div style={{ flex: 1 }}>
          <h4 style={{ margin: '0 0 2px', fontWeight: '800', fontSize: '0.92rem', color: 'var(--text-color)' }}>
            Hardware Compatibility Scan
          </h4>
          <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-muted)', lineHeight: '1.4' }}>
            Hardware specs scanning works on <strong style={{ color: 'var(--text-color)' }}>PC / Laptop browsers only</strong>. Open Gamer's Cafe on your desktop to test compatibility.
          </p>
        </div>
      </div>
    );
  ...
  // 2. DESKTOP / PC USERS - UNSCANNED STATE: SHOW SATELLITE DISH SCANNER CONSOLE
